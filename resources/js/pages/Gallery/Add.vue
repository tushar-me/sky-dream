...
onMounted(async () => {
    const data = await sendRequest({
        url: `/api/V1/gallery/${route?.params?.id}`,
        method: "GET",
    })
...
const resetOrUpdate = (data) => {
    if(data){
        ...
        createForm.value.description = data.description
    }else{
        createForm.value.name = null
        createForm.value.image = null
        createForm.value.description = null
        filepath.value = null
    }
}
...
<template>
    <AppLayout>
        <form @submit.prevent="handelCreate" class="bg-white shadow-md p-6 max-w-5xl mx-auto">
            <h3 class="mb-10 text-primary  text-xl font-medium">{{ $route?.params?.id ? 'Edit Gallery' : 'Upload Gallery' }}</h3>

            <div class="flex items-start gap-5 my-6">
                    <div class="w-1/3 pr-2">
                        <div class="w-full">
                            <label for="serviceImage" class="text-sm block mb-2">Gallery Image</label>
                    ...
                    <div class="w-full flex flex-col gap-3">
                        ...
                        <div>
                            ...
                            <textarea :disabled="loading" rows="5" v-model="createForm.description"
                            ...
                            <span class="text-red-500" v-if="error?.response?.data?.errors?.description">{{
            ...
            <div class="mt-5">
                <LoadingButton :isLoading="loading">
                    {{ $route.params?.id ? 'Update Gallery' : 'Save Gallery' }}
                </LoadingButton>
            </div>
        </form>
    </AppLayout>
